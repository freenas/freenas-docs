...
.. table:: SMB Share Options
   ...
   |                                |               |          | and is recreated whenever a file is deleted. If a file being deleted is in a subdirectory and larger than 20 MiB, adjust the                         |
   |                                |               |          | :guilabel:`Auxiliary Parameter` :command:`crossrename:sizelimit=` to a size greater than the file being deleted. For example, entering               |
   |                                |               |          | :samp:`crossrename:sizelimit=50` in the :guilabel:`Auxiliary Parameter` field increases the size limit to 50 MiB.                                    |
   +--------------------------------+---------------+----------+------------------------------------------------------------------------------------------------------------------------------------------------------+
