...
connections for connecting one or two ES12 expansion shelves to
%brand% High Availability (HA) systems are shown here. When a %brand%
unit with only a single storage controller is used, only cables #1 and
#3 are connected.
...
* Cable #2 connects to the ES12 expansion controller 2 T1 slot.

If a second ES12 is present:

* Cable #3 connects to the second ES12 expansion controller 1 T1
  slot.

* Cable #4 connects to the second ES12 expansion controller 2 T1
  slot.
