...
Drive trays have two indicator LEDs next to the tray button. The status
LED is green when the drive is active, the fault LED is amber if a
problem has occurred, and the status LED is solid green and the fault
LED blinks amber when the drive is a hot spare.
...
There are two attachment posts on the left rear side of the ES60 for
the CMA. Pulling the unit slightly out of the rack can make these posts
more accessible. Align the holes on the CMA chassis bracket with the
posts. Slide the cable management arm forward and pull the lever on the
latch upward to lock the bracket into place.
