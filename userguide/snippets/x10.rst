...
.. caution:: Two people are required to safely install the %brand%
   array in a rack or cabinet. Do not install the drives until after
   the unit has been installed in the rack.

   Two people are also required to safely remove the %brand% array
   from a rack or cabinet. Remove the drives from the %brand% unit
...
address. Remove the black USB to serial cable from the X10. Reconnect
the gray cable to the system serial console port, #10 on
:numref:`Figure %s <x10_back>`.
