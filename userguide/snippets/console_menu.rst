...
:guilabel:`1) Configure Network Interfaces` provides a configuration wizard
...
:guilabel:`2) Configure Link Aggregation` is for creating or deleting link
aggregations.
...
:guilabel:`5) Configure Static Routes` prompts for the destination network
and gateway IP address. Re-enter this option for each static route
needed.

:guilabel:`6) Configure DNS` prompts for the name of the DNS domain and the
...
:guilabel:`7) Reset Root Password` is used to reset a lost or forgotten *root*
password. Select this option and follow the prompts to set the
password.

:guilabel:`8) Reset Configuration to Defaults` *Caution*! This option deletes
...
:guilabel:`9) Shell` starts a shell for running FreeBSD commands. To leave
